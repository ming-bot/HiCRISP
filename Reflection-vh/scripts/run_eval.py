...
def planner_executer(args):
    ...
    # initialize env
    file_name1 = glob.glob(args.unity_filename)
    print(file_name1[0])
    # comm = UnityCommunication(file_name=file_name1[0], 
    #                           port=args.port, 
    #                           x_display=args.display)
    # comm = UnityCommunication(port=args.port, x_display=args.display)
    ...
    prompt.append({"role": "system", "content": f"You can only use these functions: turnright, turnleft, walkforward, walktowards <obj>, walk <obj>, run <obj>, grab <obj>, switchon <obj>, switchoff <obj>, open <obj>, close <obj>, lookat <obj>, sit <obj>, standup, find <obj>, turnto <obj>, drink <obj>, pointat <obj>, watch <obj>, putin <obj> <obj>, putback <obj> <obj>"})
    ...
    if args.prompt_task_examples == "default":
        ...
        for i in range(args.prompt_num_examples):
            ...
            prompt.append({"role": "user", "content": default_examples[i]})
            prompt.append({"role": "assistant", "content": prompt_egs[default_examples[i]]})
    ...
    if args.prompt_task_examples == "random":
        ...
        for eg in prompt_egs_keys:
            # prompt += "\n\n" + prompt_egs[eg]
            prompt.append({"role": "user", "content": eg})
            prompt.append({"role": "assistant", "content": prompt_egs[eg]})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--progprompt-path", type=str, default=r"E:\ReflectionLLMrobot\Reflection-vh")
    parser.add_argument("--expt-name", type=str, required=True)

    parser.add_argument("--openai-api-key", type=str, 
                        default="sk-xyz")
    parser.add_argument("--unity-filename", type=str, 
                        default=r"E://Progprompt/virtualhome/src/virtualhome/simulation/unity_simulator/windows_exec.v2.3.0/VirtualHome.exe")
